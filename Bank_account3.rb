...
class account
    ...
    def save_transactions
        ...
        account_file = open(filename,'a')
        account_file.write(@account['date'] + ',' + @account['name']+','+@account['bankID'] + ',' + @account['before_tr'] + ',' + @account['after_tr'] + ',' + (@account['before_tr'].to_i-@account['before_tr'].to_i) 
    end

end

    def copy_temp_to_original()
        input = File.open(tempo_report.csv)
        indata = input.read()  
        STDIN.gets
...
$account_list = Array.new
    ...
    def adding_customer
        ...
        $customer_list[$counter].save_transactions  
        $counter = $counter+1   
    end
    ...
    def adding_transaction
        ...
        $customer_list[$counter_tran].save_transactions 
        ...
        @depo_amount = gets.chomp.to_i
            ...
            CSV.foreach('customers_report.csv') do |row|
             ...
             if row[1] != @current_name
                ...
                @before_transaction = row[2].to_i
                @after_transaction = row[2].to_i + @depo_amount

                $current_date = Time.now
                $customer_list[$counter_tran] = Account.new({ 'date' => $current_date + ',' + 'name' => row[1] +','+'bankID' => row[2]+ ',' + 'before_tr' => @before_transaction + ',' + 'after_tr' => @after_transaction + ',' + 'transaction' => @depo_amount) 
    end
                customer_file.write(row[0] + "," + row[1] + "," + row[2] + "," + @after_transaction)
                customer_file.write("\n")
...
def home_page
    ...
    puts "6.viewing transaction record"
